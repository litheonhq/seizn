...
| Category | Technology | Version | Notes |
|----------|-----------|---------|-------|
| Database | PostgreSQL + pgvector | -- | Via Supabase; 138 migration files in `supabase/migrations/` |
...
| Supabase Database | Fully Implemented | `src/lib/supabase.ts`, `supabase/migrations/` (138 files) | Browser + server clients, pgvector; includes compatibility view for trace cost (`cost_usd`) + budget degrade events |
...
| Anthropic Claude Integration | Fully Implemented | `src/lib/ai.ts`, `src/lib/spring/memory-v4/` (6 files) | Memory extraction, summarization, vision |
