...
| Category | Technology | Version | Notes |
|----------|-----------|---------|-------|
| Database | PostgreSQL + pgvector | -- | Via Supabase; 184 migration files in `supabase/migrations/` |
...
| Compliance DSR | COPPA/GDPR-K subject rights | -- | `subject_id` on memories, scoped `consent_records`, `dsr_jobs`, `dsr_deletion_tombstones`, `/api/consent`, `/api/consent/[subjectId]`, `/api/consent/[subjectId]/[scope]`, `/[locale]/consent`, `/api/v1/dsr/export`, `/api/v1/dsr/delete`, `/api/v1/dsr/jobs/[id]`, `/api/dsr/jobs`, `/api/dsr/jobs/[jobId]/status`, `/api/cron/dsr-worker`, `/api/v1/audit`, and `/api/v1/audit/export` provide consent-gated memory writes, queued subject export, certified deletion, tombstone evidence, and audit evidence. |
...
| Deploy Env Guard | Next.js health route + build-time warning | -- | `src/lib/env-guard.ts` and `/api/health/env` verify production Stripe metered price/meter IDs, coupon, cron secret, and Stripe secret presence; `next.config.ts` warns during production builds without blocking deploys. |
...
| Seizn CLI | @seizn/cli + commander | 0.9.0-beta.1 | Workspace package in `cli/seizn` provides `init`, `login`, `replay`, `export`, `audit`, `bench`, and `canon list/pull/push`; credentials are stored at `~/.config/seizn/credentials.json` with `0600` permissions. |
...
| Public SLA + Status | Next.js localized pages + Supabase incidents ledger | -- | `/[locale]/sla` publishes the Pro/Enterprise 99.9% monthly uptime SLA, `/[locale]/status` shows current operational/degraded/outage state and last-30-day uptime, and `incidents` stores public incident history under RLS. |
...
| SDK Release | `sdk-release.yml` | Pushes to `main` that change OpenAPI or `packages/seizn-sdk-js`; regenerates the JS SDK, builds it, commits generated drift, and publishes beta packages with OIDC provenance when `NPM_TOKEN_PUBLISH` is configured |
...
| `openapi-typescript-codegen` | ^0.29.0 | `@seizn/sdk-js` generated fetch client source |
...
| Supabase Database | Fully Implemented | `src/lib/supabase.ts`, `supabase/migrations/` (184 files) | Browser + server clients, pgvector; includes compatibility view for trace cost (`cost_usd`) + budget degrade events + public `incidents` ledger for SLA/status reporting |
...
| Author LLM Multi-Provider (Anthropic + OpenAI, xhigh effort) | Fully Implemented | `src/lib/author/llm/provider-router.ts`, `src/lib/author/llm/anthropic-client.ts`, `src/lib/author/llm/openai-client.ts`, `src/lib/author/llm/effort-mapping.ts`, `src/lib/author/llm/byok-resolver.ts` | `AUTHOR_LLM_PROVIDER` env switches between Claude Opus 4.7 (extended-thinking 32k budget) and GPT-5.5 (`reasoning_effort:'xhigh'`). BYOK supports both providers side-by-side; 50% Stripe coupon stays applied while at least one author-stack key exists. |
...
| Stripe Payments | Partially Implemented | `src/lib/stripe-config.ts`, `src/lib/stripe-metered.ts`, `src/lib/stripe-checkout.ts`, `src/lib/env-guard.ts`, `src/app/api/webhooks/stripe/route.ts`, `src/app/api/internal/usage/flush/route.ts`, `src/app/api/billing/checkout/route.ts`, `src/app/api/health/env/route.ts`, `scripts/stripe/` | Active subscription mapping, metered overage reporting for Studio/Pro, approved Design Partner coupon application for Studio monthly checkout, and deploy-time env validation for metered billing keys. |
...
| Public SLA + Status | Fully Implemented | `src/app/[locale]/sla/page.tsx`, `src/app/[locale]/status/page.tsx`, `supabase/migrations/20260421023_incidents.sql` | Localized public SLA page covers Pro/Enterprise 99.9% monthly uptime terms; localized status page reads public incidents, computes current status and last-30-day uptime, and shows an empty state when no incidents exist. |
