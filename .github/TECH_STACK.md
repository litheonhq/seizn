...
| Build | Turbopack + opt-in Webpack analyzer | -- | Normal build uses Turbopack; `npm run analyze` writes `.next/analyze/static-bundle-report.*`; `npm run analyze:budget` enforces CI guardrails, with `SEIZN_ANALYZE_MODE=webpack` available for the heavier bundle analyzer |
...
| Fonts | Pretendard + Source Serif 4 + JetBrains Mono | -- | Self-hosted WOFF2 files in `public/fonts/` with `@font-face` tokens in `src/styles/tokens.css` |
...
| Category | Technology | Version | Notes |
|----------|-----------|---------|-------|
| Database | PostgreSQL + pgvector | -- | Via Supabase; 167 migration files in `supabase/migrations/`; extension objects are kept in the non-exposed `extensions` schema |
...
| DB Contract Verification | Migration guardrails (`run-migration-file.mjs` + DB verifiers) | -- | Every migration run triggers E2E/search-RPC, runtime primitive, and Supabase lint checks by default; fails fast on overload/RPC/RLS/grant regressions unless explicitly bypassed with `SKIP_E2E_VERIFY=1`. |
...
| AI Providers | Anthropic Claude | ^0.71.2 (SDK) | Memory extraction, summarization, vision/multimodal, Author Memory v3 Opus BYOK runtime, and structured Author candidate extraction |
...
Tailwind v4 custom properties integrate with class-based dark mode. Pretendard handles Korean/CJK UI text, Source Serif 4 carries author-facing display type, and JetBrains Mono supports code/canon chips through self-hosted WOFF2 `@font-face` assets.
...
| Author Memory v3 Eval Route | Partially Implemented | `src/lib/author/memory-v3/`, `src/lib/author/ui/`, `src/lib/author/parser/`, `src/lib/author/storage/`, `src/lib/author/llm/`, `src/lib/author/extraction/`, `src/lib/author/audit/`, `src/app/api/author/memory-v3/eval/route.ts`, `src/app/api/projects/`, `src/app/api/account/`, `src/app/(dashboard)/dashboard/author/`, `src/hooks/useAuthorMemoryV3.ts`, `supabase/migrations/20260502001_author_memory_v3_store.sql`, `supabase/migrations/20260502002_author_memory_v3_side_effect_project_scope.sql`, `supabase/migrations/20260502003_author_imports_text.sql`, `supabase/migrations/20260502004_model_usage.sql`, `supabase/migrations/20260502005_provider_keys_profile_user_id.sql`, `supabase/migrations/20260502006_author_audit_log.sql`, `supabase/migrations/20260502007_supabase_lint_hardening.sql`, `scripts/db-verify-supabase-lints.mjs`, `scripts/migrate-r2-to-litheon.sh`, `scripts/verify-r2-integrity.ts`, `docs/knot-input/`, `docs/author-ui/`, `docs/migrations/20260502-r2-litheon-migration.md` | Exposes deterministic author-canon eval payloads behind API-key auth with replay/fail-closed semantics; includes opt-in Supabase persistence for records/snapshots/project-scoped side effects/eval results, fail-closed Supabase config, KNOT v1/v2/v3 eval seed ingestion, v3 100-case record/replay fixture runner, optional Anthropic judge verifier, Fall dataset import helper, Author UI contract/query binding artifacts locked by tests, fixture-backed Author UI API routes, strict CSRF token handling, SWR hooks, polling fallback, dashboard upload control, Phase 1 Cloudflare R2 import persistence, md/docx/pdf/txt parser pipeline, durable parsed import text storage, Phase 2 Anthropic BYOK runtime, model usage ledger, profile-ID-aligned BYOK storage, Phase 3 extraction prompts/schemas/canon validator/eval harness wired into import candidate creation, Phase 4 character backlog generation API/hook/UI with Review Queue insertion and detail-guide export markdown, Phase 5 persistent audit log/replay chain API plus dashboard audit viewer and DB-side search RPC, and Phase 6 R2 migration preflight tooling; Litheon bucket/account readiness remains the blocker for execution |
