...
export async function resolveSaveFileContext(request: NextRequest): Promise<
  ...
  if (!organizationId) {
    return {
      error: NextResponse.json(
        { success: false, error: { code: 'organization_required', message: 'No organization is available for save-file operations' } },
        { status: 400 }
      ),
    };
  }
