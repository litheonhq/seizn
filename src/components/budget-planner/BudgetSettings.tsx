  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--ink-0)] rounded-lg border border-[var(--ink-200)] p-6">
        <h3 className="text-lg font-semibold text-[var(--ink-900)] mb-4">Budget Limits</h3>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Daily Budget */}
          <div>
            <label htmlFor="budget-daily" className="block text-sm font-medium text-[var(--ink-600)] mb-1">
              Daily Budget (USD)
            </label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ink-600)]">$</span>
              <input
                id="budget-daily"
                type="number"
                step="0.01"
                ...
                className="w-full pl-7 pr-3 py-2 border border-[var(--ink-200)] rounded-md bg-[var(--ink-0)] text-[var(--ink-900)] placeholder-[var(--ink-500)] focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 disabled:bg-[var(--ink-50)]"
              />
            </div>
            <p className="text-xs text-[var(--ink-600)] mt-1">Max spend per day</p>
          </div>

          {/* Monthly Budget */}
          <div>
            <label htmlFor="budget-monthly" className="block text-sm font-medium text-[var(--ink-600)] mb-1">
              Monthly Budget (USD)
            </label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ink-600)]">$</span>
            ...
            <p className="text-xs text-[var(--ink-600)] mt-1">Max spend per month</p>
          </div>

          {/* Per-Query Max */}
          <div>
            <label htmlFor="budget-per-query" className="block text-sm font-medium text-[var(--ink-600)] mb-1">
              Per-Query Max (USD)
            </label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ink-600)]">$</span>
            ...
            <p className="text-xs text-[var(--ink-600)] mt-1">Max cost per query</p>
          </div>
        </div>
      </div>
      ...
      {/* Alert Settings */}
      <div className="bg-[var(--ink-0)] rounded-lg border border-[var(--ink-200)] p-6">
        <h3 className="text-lg font-semibold text-[var(--ink-900)] mb-4">Alert Settings</h3>

        <div>
          <label htmlFor="budget-alert-threshold" className="block text-sm font-medium text-[var(--ink-600)] mb-1">
            Alert Threshold
          </label>
          <div className="flex items-center gap-4">
            ...
            />
            <span className="text-sm font-medium text-[var(--ink-900)] w-12">
              {form.alertAtPercent}%
            </span>
          </div>
      ...
      {/* Enforcement Mode */}
      <div className="bg-[var(--ink-0)] rounded-lg border border-[var(--ink-200)] p-6">
        <h3 className="text-lg font-semibold text-[var(--ink-900)] mb-4">Enforcement Mode</h3>
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 dark:text-blue-400"
                />
                <span className="text-sm text-[var(--ink-900)]">Hard</span>
              </label>
            </div>
            <p className="text-xs text-[var(--ink-600)] mt-1">
              {form.mode === "soft"
          ...
          {form.mode === "soft" && (
            <div>
              <label htmlFor="budget-fallback-strategy" className="block text-sm font-medium text-[var(--ink-600)] mb-2">
                Fallback Strategy
              </label>
              <select
                ...
                disabled={disabled || saving}
                className="w-full px-3 py-2 border border-[var(--ink-200)] rounded-md bg-[var(--ink-0)] text-[var(--ink-900)] focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 disabled:bg-[var(--ink-50)]"
              >
      ...
      {success && (
        <div className="p-4 bg-[var(--signal-canon-soft)] dark:bg-[var(--signal-canon-ink)]/25 border border-[var(--signal-canon)] dark:border-[var(--signal-canon)]/40 rounded-lg">
          <p className="text-sm text-[var(--signal-canon-ink)] dark:text-[var(--signal-canon-soft)]">Settings saved successfully!</p>
        </div>
      )}
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={disabled || saving}
          className="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:ring-offset-2 dark:focus:ring-offset-gray-950 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
