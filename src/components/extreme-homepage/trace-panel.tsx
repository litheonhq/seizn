...
export function TracePanel({ trace, isLoading, translations: t }: TracePanelProps) {
  ...
  return (
    <div className="space-y-4 h-full overflow-auto max-h-[calc(100%-2rem)]">
      {/* Summary Bar */}
      <div className="flex items-center justify-between p-4 bg-[var(--ink-50)] rounded-xl">
        <div className="flex items-center gap-6">
          <div>
            <div className="text-xs text-[var(--ink-600)] mb-1">{t?.latency || "Latency"}</div>
            <div className="text-lg font-semibold text-[var(--ink-900)]">
              {trace.totalLatencyMs.toFixed(0)}ms
            </div>
          </div>
          <div className="w-px h-10 bg-[var(--ink-200)]" />
          <div>
            <div className="text-xs text-[var(--ink-600)] mb-1">{t?.cost || "Cost"}</div>
            <div className="text-lg font-semibold text-[var(--ink-900)]">
          ...
          <div className="w-px h-10 bg-[var(--ink-200)]" />
          <div>
            <div className="text-xs text-[var(--ink-600)] mb-1">{t?.tokens || "Tokens"}</div>
            <div className="text-lg font-semibold text-[var(--ink-900)]">
          ...
          <div className="w-px h-10 bg-[var(--ink-200)]" />
          <div>
            <div className="text-xs text-[var(--ink-600)] mb-1">{t?.vectorOps || "Vector Ops"}</div>
            <div className="text-lg font-semibold text-[var(--ink-900)]">
              {trace.vectorOps}
            </div>
          </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <svg
                        className={`w-4 h-4 text-[var(--ink-500)] transition-transform ${isExpanded ? "rotate-180" : ""}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
