...
export function RequestBuilder({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Query Input */}
      <div className="mb-4">
        <label htmlFor={queryId} className="block text-sm font-medium text-[var(--ink-900)] mb-2">
          {t?.query || "Query"}
        </label>
        <div className="relative">
          <textarea
            ...
            className={`w-full border border-[var(--ink-200)] rounded-xl focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-[var(--ink-200)] resize-none transition-all text-[var(--ink-900)] placeholder:text-[var(--ink-500)] ${
          ...
          <button
            ...
            className="absolute right-3 bottom-3 text-xs text-[var(--ink-500)] hover:text-[var(--ink-600)] transition-colors"
          >
            {t?.tryExamples || "Try examples"}
          </button>
        </div>
        {showSampleQueries && (
          <div className="mt-2 p-3 bg-[var(--ink-50)] rounded-lg space-y-2">
            {SAMPLE_QUERIES.map((q, i) => (
              <button
                ...
                className="block w-full text-left text-sm text-[var(--ink-600)] hover:text-black hover:bg-[var(--ink-0)] px-3 py-2 rounded-lg transition-colors"
              >
                {q}
              </button>
            ))}
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            className={`rounded-xl border text-left transition-all ${
              currentPreset === "fast"
                ? "border-[var(--ink-900)] bg-[var(--ink-900)]/10 ring-1 ring-[var(--ink-900)]"
                : "border-[var(--ink-200)] hover:border-[var(--ink-200)] hover:bg-[var(--ink-50)]"
            } ${compact ? "p-2.5" : "p-3"}`}
            disabled={disabled}
          >
            <div className="flex items-center gap-1.5 mb-1">
              <svg className={`w-4 h-4 ${currentPreset === "fast" ? "text-[var(--ink-900)]" : "text-[var(--ink-500)]"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              <span className={`text-sm font-medium ${currentPreset === "fast" ? "text-[var(--ink-900)]" : "text-[var(--ink-900)]"}`}>
                {t?.presets?.fast || "Fast"}
              </span>
            </div>
            {!compact && <p className="text-xs text-[var(--ink-600)]">{t?.presets?.fastDesc || "~200ms, basic"}</p>}
          </button>
          ...
          >
            <div className="flex items-center gap-1.5 mb-1">
              <svg className={`w-4 h-4 ${currentPreset === "balanced" ? "text-blue-600" : "text-[var(--ink-500)]"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" />
              </svg>
              <span className={`text-sm font-medium ${currentPreset === "balanced" ? "text-blue-700" : "text-[var(--ink-900)]"}`}>
                {t?.presets?.balanced || "Balanced"}
              </span>
            </div>
            {!compact && <p className="text-xs text-[var(--ink-600)]">{t?.presets?.balancedDesc || "~500ms, rerank"}</p>}
          </button>
          ...
          <button
            ...
            className={`rounded-xl border text-left transition-all ${
              currentPreset === "precise"
                ? "border-[var(--ink-900)] bg-[var(--ink-50)] ring-1 ring-purple-500"
                : "border-[var(--ink-200)] hover:border-[var(--ink-200)] hover:bg-[var(--ink-50)]"
            } ${compact ? "p-2.5" : "p-3"}`}
            disabled={disabled}
          >
            <div className="flex items-center gap-1.5 mb-1">
              <svg className={`w-4 h-4 ${currentPreset === "precise" ? "text-[var(--ink-900)] underline" : "text-[var(--ink-500)]"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
              </svg>
              <span className={`text-sm font-medium ${currentPreset === "precise" ? "text-[var(--ink-900)] underline" : "text-[var(--ink-900)]"}`}>
                {t?.presets?.precise || "Precise"}
              </span>
            </div>
            {!compact && <p className="text-xs text-[var(--ink-600)]">{t?.presets?.preciseDesc || "~1.5s, full suite"}</p>}
          </button>
        </div>
      </div>
      ...
      <div className="mb-4">
        <button
          ...
          className="flex items-center justify-between w-full py-2 text-sm font-medium text-[var(--ink-600)] hover:text-[var(--ink-900)] transition-colors"
        >
        ...
        {/* Advanced Content */}
        {showAdvanced && (
          <div className="mt-3 p-4 bg-[var(--ink-50)] rounded-xl space-y-4">
            {/* Dataset Selector */}
            <div>
              <label htmlFor={datasetId} className="block text-sm font-medium text-[var(--ink-900)] mb-2">
                {t?.dataset || "Dataset"}
              </label>
              <div className="relative">
                <select
                  ...
                  className="w-full px-4 py-2.5 pr-10 border border-[var(--ink-200)] rounded-lg focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-[var(--ink-200)] bg-[var(--ink-0)] transition-all text-[var(--ink-900)] text-sm appearance-none cursor-pointer"
                  disabled={disabled}
                >
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label htmlFor={budgetId} className="text-sm font-medium text-[var(--ink-900)]">
                  {t?.latencyBudget || "Latency Budget"}
                </label>
                <span className="text-sm text-[var(--ink-600)]">{config.budgetMs}ms</span>
              </div>
              <input
                id={budgetId}
                type="range"
                ...
                className="w-full h-2 bg-[var(--ink-50)] rounded-lg appearance-none cursor-pointer accent-black"
                aria-describedby={budgetHelpId}
                disabled={disabled}
              />
              <div id={budgetHelpId} className="flex justify-between text-xs text-[var(--ink-500)] mt-1">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label htmlFor={topKId} className="text-sm font-medium text-[var(--ink-900)]">{t?.topK || "Top K"}</label>
                <span className="text-sm text-[var(--ink-600)]">{config.topK} {t?.results || "results"}</span>
              </div>
              <input
                id={topKId}
                type="range"
      ...
      <div className="mb-6 space-y-3">
        ...
        <label className="flex items-center justify-between p-3 bg-[var(--ink-50)] rounded-xl cursor-pointer hover:bg-[var(--ink-50)] transition-colors">
          <div>
          ...
          <input aria-label="Hybrid Search"
            ...
            className="w-5 h-5 rounded border-[var(--ink-200)] text-black focus:ring-black/20 accent-black"
            disabled={disabled}
          />
        </label>
      ...
      {compact ? (
        <div className="mb-5 px-3 py-2 rounded-lg bg-[var(--ink-50)] border border-[var(--ink-200)] text-xs text-[var(--ink-600)]">
        ...
        {/* Latency Card */}
        <div className="p-3 bg-gradient-to-br from-[var(--ink-50)] to-[var(--ink-100)] rounded-xl border border-blue-100">
        ...
        {/* Cost Card */}
        <div className="p-3 bg-gradient-to-br from-[var(--ink-900)]/10 to-[var(--ink-700)]/10 rounded-xl border border-[var(--ink-900)]/20">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-[var(--ink-900)]">
              {t?.estimatedCost || "Est. Cost"}
            </span>
          </div>
          <div className="space-y-1">
            <div className="flex items-baseline justify-between">
              <span className="text-xs text-[var(--ink-900)]/70">{t?.perRequest || "/req"}</span>
              <span className="text-lg font-bold text-[var(--ink-900)]">${estimates.costPerRequest.toFixed(4)}</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-xs text-[var(--ink-900)]/70">{t?.per1000 || "/1K"}</span>
              <span className="text-sm font-medium text-[var(--ink-900)]">${estimates.costPer1000.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className={`w-full bg-black text-white font-medium rounded-xl hover:bg-[var(--ink-800)] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 ${
          compact ? "py-3.5" : "py-4"
        }`}
      >
        {isLoading ? (
