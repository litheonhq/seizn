...
export function ReceiptCard({
  ...
  const statusColor = useMemo(() => {
    if (receipt.policy.budget_remaining_percent < 10) return "text-[var(--signal-conflict-soft)]";
  ...
  if (compact) {
    return (
      <button
        onClick={onClick}
        className={`w-full p-3 bg-[var(--ink-900)] rounded-lg border border-[var(--ink-200)] hover:border-[var(--ink-200)] transition-colors text-left ${className}`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm font-mono text-[var(--ink-500)]">
              {receipt.receipt_id.slice(0, 12)}...
            </span>
            {cacheHitBadge}
          </div>
          <div className="flex items-center gap-4 text-sm">
            <span className="text-[var(--ink-900)]">{formatCost(receipt.cost.estimated_cost_usd)}</span>
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full bg-[var(--ink-900)] rounded-lg border border-[var(--ink-200)] hover:border-[var(--ink-200)] transition-colors text-left overflow-hidden ${className}`}
    >
      {/* Header */}
      <div className="p-4 border-b border-[var(--ink-200)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-mono text-[var(--ink-500)]">{receipt.receipt_id}</span>
            {cacheHitBadge}
          </div>
        ...
        {receipt.query_preview && (
          <p className="mt-2 text-sm text-[var(--ink-600)] truncate">
      ...
      {/* Metrics Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-[var(--ink-200)]">
        {/* Cost */}
        <div className="p-4">
          <div className="text-xs text-[var(--ink-600)] uppercase tracking-wide mb-1">Cost</div>
        ...
        {/* Latency */}
        <div className="p-4">
          <div className="text-xs text-[var(--ink-600)] uppercase tracking-wide mb-1">Latency</div>
          <div className="text-xl font-bold text-blue-400">
            {formatLatency(receipt.execution.latency_ms)}
          </div>
          <div className="text-xs text-[var(--ink-600)] truncate" title={receipt.execution.plan_path}>
        ...
        {/* Evidence */}
        <div className="p-4">
          <div className="text-xs text-[var(--ink-600)] uppercase tracking-wide mb-1">Evidence</div>
          <div className="text-xl font-bold text-[var(--ink-700)]">
        ...
        {/* Policy */}
        <div className="p-4">
          <div className="text-xs text-[var(--ink-600)] uppercase tracking-wide mb-1">Policy</div>
      ...
      {/* Footer */}
      <div className="px-4 py-2 bg-[var(--ink-800)]/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          {receipt.plan && (
        ...
        <svg
          className="w-4 h-4 text-[var(--ink-600)]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
