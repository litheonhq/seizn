...
- 2026-05-03 — **Phase D″ self-audit + fix 완료** commit `4557c7b0` — P0×0·P1 해결(Checkout legal bypass 차단·`legalAccepted` + terms/privacy version Stripe metadata 기록·Stripe `idempotencyKey` race 완화·Landing footer Privacy/Terms/Beta Disclosure 복구·Mobile nav inert 버튼 + `aria-expanded`·detector `aria-live` + input `aria-pressed`·canon graph 'one-click/open ledger' 과장 정리·CSP font allowlist 보강·Mark A favicon PNG/ICO fallback 재생성)·P2 일부 해결(BYOK 카피를 'Anthropic key'로 좁힘·detector touch target 44px). 1171 tests 복구 (+8)·typecheck/lint/build pass·KNOT separation 0. 잔여(legacy cyan 전역 grep + custom font warning)는 별 cycle. **launch readiness 100%·founding member outreach 진입 가능**
- 2026-05-03 — **Dogfood 진입 시 sign-in 페이지 brand 잔재 발견** — 사용자 스크린샷: 로고 violet/purple/cyan 그라데이션 'S' 박스·Sign In 버튼 동일 그라데이션·`szn-*` 옛 토큰·decorative blob·Cloudflare Turnstile 위젯 로드 실패 (별 이슈). Phase D″ scope = landing 9 sections 한정·auth 페이지 누락 확인. self-contained handoff doc 작성 commit `3e7a04e8` (`docs/architecture/seizn-author-auth-token-migration-handoff.md`)·Phase A~E 단계별 변경·token 매핑표·검증 grep 룰 포함
- 2026-05-03 — **Auth token migration (Phase D″ scope 확장) Codex 완료** commit `9aaaaea6` — `auth/layout.tsx` Geist 제거 + `var(--font-sans)` (Pretendard)·login/signup/device 3 폼 V1 정합 (Mark A `<SeiznLockup variant="graph" tone="dark" size="md" />`·monochrome `var(--ink-*)` 토큰·signal-canon/conflict/pending 매핑·decorative blob 제거·`auth-btn-primary/secondary/oauth-github` utility class 신설). 1171 tests pass (Phase D″ baseline 유지)·typecheck/lint/build pass·knot-separation 0 matches·auth scope grep (옛 violet/purple/cyan/szn-* token) 0 hits. Cloudflare Turnstile 에러는 별 사용자 action (Vercel preview env에서 `NEXT_PUBLIC_TURNSTILE_SITE_KEY` 제거)
